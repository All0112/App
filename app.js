...
// This simulates Supabase functionality locally since demo.supabase.co doesn't exist
class MockSupabaseClient {
    constructor() {
        this.users = new Map(); // Simulate database in memory
        ...
        return {
            ...
            insert: (data) => ({
                select: () => ({
                    single: async () => {
                        await this.simulateDelay();
                        if (table === 'usuarios' && Array.isArray(data) && data.length > 0) {
                            const userData = data[0];
                            const newUser = {
                                id: this.users.size + 1,
                                username: userData.username,
                                dados_orcamento: userData.dados_orcamento || {},
                    ...
                    async single() {
                        ...
                        if (table === 'usuarios') {
                            ...
                            if (user) {
                                ...
                                this.users.set(value, updatedUser);
